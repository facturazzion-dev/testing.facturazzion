        ...
        <div class="card-body">
            <div class="alert alert-warning">
                @if(isset($active_subscription->subscription_type) && $active_subscription->subscription_type=='paypal')
                    Si el monto del plan que cambió es un 120% mayor que el plan anterior, se creará una nueva suscripción.
                @else
                    El cambio de planes eliminará la versión de prueba.
                @endif
            </div>
                ...
                <table id="data" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$active_plan->amount}} {!! $active_plan->currency !!}</td>
                            <td>
                        ...
                        <table id="invoice_data" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach (count($subscriptions->data) > 100 ? $subscriptions->autoPagingIterator() : $subscriptions->data as $data)
                                    <tr>
                                        <td>
                                            {{ '$'.number_format(($data->total),2).' MXN' }}
                                        </td>
                                        <td>
                                            @if(isset($data->paid))
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                @elseif($event->type=='charge.succeeded')
                                                    ...
                                                    {{ $subscription_customer->email }} was charged {{ '$'.number_format(($event->data->object->amount),2) }}

                                                @elseif($event->type=='invoice.payment_succeeded')

                                                    {{ $subscription_customer->email.'\'s invoice for $'.number_format(($event->data->object->amount_due),2).' was paid' }}

                                                @elseif($event->type=='invoice.created')

                                                    {{ $subscription_customer->email.' has a new invoice for $'.number_format(($event->data->object->amount_due),2) }}
                                                ...
                                                @elseif($event->type=='invoiceitem.created')
                                                    ...
                                                    @if($event->data->object->amount<0)
                                                        {{ 'A proration adjustment for was created for ($'.(-($event->data->object->amount)).') '. $subscription_customer->email}}
                                                    @else
                                                        {{ 'A proration adjustment for was created for ($'.($event->data->object->amount).') '. $subscription_customer->email}}
                                                    @endif
