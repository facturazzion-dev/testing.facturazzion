...
<div class="card document-add">
  <div class="card-body document-preview-card">
    ...
    @else
    {!! Form::model($quotation, ['url' => 'quotation' , 'method' => 'post', 'files'=> true, 'id'=>'form']) !!}
    @endif
    @else
    ...
    <div class="row">
      <div class="col-md-6">
        <div class="form-group required {{ $errors->has('company_id') ? 'has-error' : '' }}">
          ...
          {!! Form::select('company_id', isset($companies)?$companies:null, $quotation->company_id ?? $company_fav ?? null, ['class' => 'form-select company', 'placeholder' => 'Seleccionar cliente']) !!}
          <span class="help-block">{{ $errors->first('company_id', ':message') }}</span>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group required {{ $errors->has('quotation_serie') ? 'has-error' : '' }}">
          {!! Form::label('quotation_serie', 'Serie', ['class' => 'form-label required']) !!}
          {!! Form::text('quotation_serie', isset($quotation->quotation_serie)?$quotation->quotation_serie:$quotation_serie , ['class' => 'form-control']) !!}
          <span class="help-block">{{ $errors->first('quotation_serie', ':message') }}</span>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group required {{ $errors->has('quotation_number') ? 'has-error' : '' }}">
          {!! Form::label('quotation_number', 'Folio', ['class' => 'form-label required']) !!}
          {!! Form::number('quotation_number', (isset($quotation->quotation_number) && $quotation->quotation_number != '')?$quotation->quotation_number:$quotation_number , ['class' => 'form-control']) !!}
          <span class="help-block">{{ $errors->first('quotation_number', ':message') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-lg-3">
        <div class="form-group required {{ $errors->has('currency') ? 'has-error' : '' }}">
          ...
          {!! Form::select('currency', array('MXN'=>'Peso mexicano', 'USD' => 'Dólar americano'), isset($quotation->currency)?$quotation->currency:'MXN' , ['class' => 'form-select']) !!}
      ...
      <div class="col-lg-2 d-none" id="currency-group">
        <div class="form-group  {{ $errors->has('exchange_rate') ? 'has-error' : '' }}">
          ...
          {!! Form::text('exchange_rate', isset($quotation->exchange_rate)?$quotation->exchange_rate:'1', ['onClick' => "SelectAll('exchange_rate')", 'class' => 'form-control']) !!}
      ...
      <div class="col-lg-3">
        <div class="form-group  {{ $errors->has('payment_term') ? 'has-error' : '' }}">
          ...
          <select name="payment_term" class="form-select" id="payment_term">
            <option value="0 {{trans('quotation.days')}}" @if(isset($quotation) && $quotation->payment_term.' '.trans('quotation.days')==0) selected @endif>{{trans('quotation.immediate_payment')}}</option>
            @if($payment_term1!='0' && $payment_term1!='')
            <option value="{{$payment_term1.' '.trans('quotation.days')}}" @if(isset($quotation) && $payment_term1.' '.trans(' quotation.days')==$quotation->payment_term) selected @endif>{{$payment_term1}} {{trans('quotation.days')}}</option>
            @endif
            @if($payment_term2!='0' && $payment_term2!='')
            <option value="{{$payment_term2.' '.trans('quotation.days')}}" @if(isset($quotation) && $payment_term2.' '.trans(' quotation.days')==$quotation->payment_term) selected @endif>{{$payment_term2}} {{trans('quotation.days')}}</option>
            @endif
            @if($payment_term3!='0' && $payment_term3!='')
            <option value="{{$payment_term3.' '.trans('quotation.days')}}" @if(isset($quotation) && $payment_term3.' '.trans(' quotation.days')==$quotation->payment_term) selected @endif>{{$payment_term3}} {{trans('quotation.days')}}</option>
            @endif
          </select>
      ...
      <div data-repeater-list="products">
        @if(isset($quotation))
            @foreach ($quotation->products as $product)
                @include('content._partials._forms.form-product-repeater', [ 'docTaxes' => $quotation->taxes, 'backup_product' => $product, 'product' => $product->pivot, 'count' => $loop->index + 1])
            @endforeach
        @else
...
<footer class="footer footer-light footer-fixed">
  <p class="clearfix mb-0 d-flex align-items-center justify-content-md-end">
    <button type="button" id="quotation-preview" class="btn btn-outline-primary ms-1">Previsualizar</button>
    <button type="button" id="quotation-draft" class="btn btn-outline-primary ms-1">Guardar</button>
    <button type="button" id="quotation-create" class="btn btn-primary ms-1">Crear</button>
  </p>
</footer>
@endsection
    ...
    $('#quotation-create').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, xhr) {
                ...
                sendQuotationModal.find('.hidden-document-id').val(quotation_id);
