...
<div class="card document-add">
  <div class="card-body document-preview-card">
    @if (isset($saleorder))
    @if ($saleorder->id != 0)
    {!! Form::model($saleorder, ['url' => $type . '/' . $saleorder->id, 'method' => 'put', 'files'=> true, 'id'=>'form']) !!}
    @else
    {!! Form::model($saleorder, ['url' => $type , 'method' => 'post', 'files'=> true, 'id'=>'form']) !!}
    @endif
    @else
    ...
    <div class="row">
      <div class="col-md-6">
        <div class="form-group required {{ $errors->has('company_id') ? 'has-error' : '' }}">
          {!! Form::label('company_id', trans('sales_order.company_id'), ['class' => 'form-label required']) !!}
          {!! Form::select('company_id', isset($companies)?$companies:null, $saleorder->company_id ?? $company_fav ?? null, ['class' => 'form-select company', 'placeholder' => 'Seleccionar cliente']) !!}
          <span class="help-block">{{ $errors->first('company_id', ':message') }}</span>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group required {{ $errors->has('sale_serie') ? 'has-error' : '' }}">
          {!! Form::label('sale_serie', 'Serie', ['class' => 'form-label required']) !!}
          {!! Form::text('sale_serie', isset($saleorder->sale_serie)?$saleorder->sale_serie:$sale_serie , ['class' => 'form-control']) !!}
          <span class="help-block">{{ $errors->first('sale_serie', ':message') }}</span>
        </div>
      </div>
      <div class="col-md-2">
        <div class="form-group required {{ $errors->has('sale_number') ? 'has-error' : '' }}">
          {!! Form::label('sale_number', 'Folio', ['class' => 'form-label required']) !!}
          {!! Form::number('sale_number', (isset($saleorder->sale_number) && $saleorder->sale_number != '')?$saleorder->sale_number:$sale_number , ['class' => 'form-control']) !!}
          <span class="help-block">{{ $errors->first('sale_number', ':message') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-lg-3">
        <div class="form-group required {{ $errors->has('currency') ? 'has-error' : '' }}">
          ...
          {!! Form::select('currency', array('MXN'=>'Peso mexicano', 'USD' => 'Dólar americano'), isset($saleorder->currency)?$saleorder->currency:'MXN' , ['class' => 'form-select']) !!}
      ...
      <div class="col-lg-2 d-none" id="currency-group">
        <div class="form-group  {{ $errors->has('exchange_rate') ? 'has-error' : '' }}">
          ...
          {!! Form::text('exchange_rate', isset($saleorder->exchange_rate)?$saleorder->exchange_rate:'1', ['onClick' => "SelectAll('exchange_rate')", 'class' => 'form-control']) !!}
      ...
      <div class="col-lg-3">
        <div class="form-group  {{ $errors->has('payment_term') ? 'has-error' : '' }}">
          ...
          <select name="payment_term" class="form-select" id="payment_term">
            <option value="0 {{trans('sales_order.days')}}" @if(isset($saleorder) && $saleorder->payment_term.' '.trans('sales_order.days')==0) selected @endif>{{trans('sales_order.immediate_payment')}}</option>
            @if($payment_term1!='0' && $payment_term1!='')
            <option value="{{$payment_term1.' '.trans('sales_order.days')}}" @if(isset($saleorder) && $payment_term1.' '.trans(' sales_orders.days')==$saleorder->payment_term) selected @endif>{{$payment_term1}} {{trans('sales_order.days')}}</option>
            @endif
            @if($payment_term2!='0' && $payment_term2!='')
            <option value="{{$payment_term2.' '.trans('sales_order.days')}}" @if(isset($saleorder) && $payment_term2.' '.trans(' sales_orders.days')==$saleorder->payment_term) selected @endif>{{$payment_term2}} {{trans('sales_order.days')}}</option>
            @endif
            @if($payment_term3!='0' && $payment_term3!='')
            <option value="{{$payment_term3.' '.trans('sales_order.days')}}" @if(isset($saleorder) && $payment_term3.' '.trans(' sales_orders.days')==$saleorder->payment_term) selected @endif>{{$payment_term3}} {{trans('sales_order.days')}}</option>
            @endif
          </select>
      ...
      <div data-repeater-list="products">
        @if(isset($saleorder))
            @foreach ($saleorder->products as $product)
                @include('content._partials._forms.form-product-repeater', [ 'docTaxes' => $saleorder->taxes, 'backup_product' => $product, 'product' => $product->pivot, 'count' => $loop->index + 1])
            @endforeach
        @else
...
<footer class="footer footer-light footer-fixed">
  <p class="clearfix mb-0 d-flex align-items-center justify-content-md-end">
    <button type="button" id="saleorder-preview" class="btn btn-outline-primary ms-1">Previsualizar</button>
    <button type="button" id="saleorder-draft" class="btn btn-outline-primary ms-1">Guardar</button>
    <button type="button" id="saleorder-create" class="btn btn-primary ms-1">Crear</button>
  </p>
</footer>
@endsection
...
<script>
  $(function () {
    ...
    $('#saleorder-preview').click(function (e) {
        e.preventDefault();
        syncTotalValues();
    ...
    $('#saleorder-draft').click(function (e) {
        syncTotalValues();
        $("#form").append('<input type="hidden" name="status" value="Borrador" />');
        $("#form").submit();
    });
    $('#saleorder-create').click(function (e) {
        syncTotalValues();
        processingModal.modal('show');
        $(".input-disable").prop('disabled', false);
        var form = $("#form"),
        ...
        $.ajax({
            ...
            contentType: false,
            success: function (data, textStatus, xhr) {
                var { saleorder_id } = data,
                    companyEmail = $('#email').val(),
                    iframePdfPath = basePath + saleorder_id + '/print_quot',
                    downloadPdfPath = basePath + saleorder_id + '/download_pdf';

                sendSaleorderModal.find('.hidden-document-id').val(saleorder_id);
    ...
    /*Initilizes saleorder totals on page load*/
    $('.price').keyup();
    ...
    sendSaleorderModal.on('hide.bs.modal', function () {
