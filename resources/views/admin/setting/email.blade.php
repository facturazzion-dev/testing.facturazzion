...
@section('scripts')
    <script>
        $(document).ready(function($) {
            ...
            $(".gofac_live,.gofac_sandbox").hide();
            if('{{ isset($settings['gofac_mode']) && !empty($settings['gofac_mode']) }}'){
                if('{{ isset($settings['gofac_mode']) && $settings['gofac_mode']=='sandbox' }}'){
                    $(".gofac_sandbox").show();
                }else{
                    $(".gofac_live").show();
                }
            }
            if('{{ old('gofac_mode')=='sandbox' }}'){
                $(".gofac_sandbox").show();
                $(".gofac_live").hide();
            }
            if('{{ old('gofac_mode')=='live' }}'){
                $(".gofac_sandbox").hide();
                $(".gofac_live").show();
            }
            $(".sandbox").on("ifChecked",function(){
            ...
            $(".gofac,.paybook_sync").hide();
            if('{{ isset($settings['timbrador_selected']) && !empty($settings['timbrador_selected']) }}'){
                if('{{ isset($settings['timbrador_selected']) && $settings['timbrador_selected']=='paybook_sync' }}'){
                    $(".paybook_sync").show();
                }else{
                    $(".gofac").show();
                }
            }
            if('{{ old('timbrador_selected')=='paybook_sync' }}'){
                $(".paybook_sync").show();
                $(".gofac").hide();
            }
            if('{{ old('timbrador_selected')=='gofac' }}'){
                $(".paybook_sync").hide();
                $(".gofac").show();
            }
            $(".paybook_sync").on("ifChecked",function(){
                $(".paybook_sync").show();
                $(".gofac").hide();
            });
            $(".paybook_sync").on("ifUnchecked",function(){
                $(".paybook_sync").hide();
                $(".gofac").show();
            });
        });
